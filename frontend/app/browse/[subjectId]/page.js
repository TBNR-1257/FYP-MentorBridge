  ...
  return (
    <main className="flex flex-1 flex-col items-center gap-6 px-6 py-10">
      <div className="w-full max-w-2xl">
        <Link href="/browse" className="text-sm text-[#9fb8ae] hover:underline">
          &larr; Back to browse
        </Link>
        ...
        <div className="mt-4 flex rounded-lg border border-[#2c4a40] bg-[#102420] p-1 text-sm">
          <button
            ...
            className={`rounded px-4 py-1.5 ${tab === "courses" ? "bg-gradient-to-r from-[#12796f] to-[#6FE9DC] text-white" : ""}`}
          >
            Courses
          </button>
          <button
        ...
        <input
          ...
          className="mt-4 w-full rounded-lg border border-[#2c4a40] bg-[#102420] px-3 py-2 text-sm"
        />
      </div>
      ...
      <section className="flex w-full max-w-2xl flex-col gap-3">
        {fetching ? (
          ...
          mentors.length === 0 ? (
            <p className="text-sm text-[#9fb8ae]">No mentors found.</p>
          ) : (
            mentors.map((mentor) => (
              <MentorCard
                key={mentor.id}
          ...
          <p className="text-sm text-[#9fb8ae]">No courses found.</p>
        ) : (
...
function CourseCard({ course, token, router }) {
  ...
  return (
    <div className="rounded-lg border border-[#234339] bg-[#102420] p-4 text-sm">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="shrink-0 text-xs text-[#9fb8ae]">{course._count.enrollments} enrolled</span>
      </div>
      <p className="mt-2 text-[#9fb8ae]">{course.description}</p>
      {course.timeSlots.length > 0 && (
        <p className="mt-2 text-xs text-[#9fb8ae]">
          {course.timeSlots
      ...
      <button
        onClick={handleJoin}
        disabled={joining}
        className="mt-3 rounded-lg bg-gradient-to-r from-[#12796f] to-[#6FE9DC] px-3 py-1.5 text-xs text-white hover:opacity-90 disabled:opacity-50"
      >
...
function MentorCard({ mentor, token, subjectId, helpRequestIdFromLink, openRequestsForSubject, router }) {
  ...
  return (
    <div className="rounded-lg border border-[#234339] bg-[#102420] p-4 text-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <Link href={`/mentors/${mentor.id}`} className="font-medium text-[#e7f0ed] hover:underline">
            {mentor.name}
          </Link>
          <p className="text-[#9fb8ae]">
      ...
      {mentor.bio && <p className="mt-2 text-[#9fb8ae]">{mentor.bio}</p>}
      <p className="mt-2 text-xs text-[#9fb8ae]">{mentor.qualifications}</p>
      {mentor.languages.length > 0 && (
        <p className="mt-1 text-xs text-[#9fb8ae]">Languages: {mentor.languages.join(", ")}</p>
      )}

      {error && <p className="mt-2 text-xs text-red-400">{error}</p>}

      <div className="mt-3 border-t border-[#1a2e28] pt-3">
        {helpRequestIdFromLink ? (
          ...
          <Link
            ...
            className="text-xs text-[#39C5BB] hover:underline"
          >
          ...
          <div className="flex items-center gap-2">
            {openRequestsForSubject.length > 1 && (
              <select
                ...
                className="rounded-lg border border-[#2c4a40] bg-[#102420] px-2 py-1 text-xs"
              >
